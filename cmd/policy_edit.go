...
var policyEditCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		case *pango.Firewall:
			...
			if t == "security" {
				...
				for _, rule := range rules {
					...
					if len(strings.TrimSpace(rule[2])) > 63 {
						log.Printf("failed to edit %s: name is over the max 63 characters", strings.TrimSpace(rule[0]))
					} else {
						e := security.Entry{
						...
						if err != nil {
							if strings.Contains(err.Error(), "Client.Timeout") {
								...
								log.Printf("failed to edit Security rule: %s", err)
							}
						}
					}
					...
					if len(strings.TrimSpace(rule[0])) > 63 {
						...
						if err != nil {
							if strings.Contains(err.Error(), "Client.Timeout") {
								...
								log.Printf("failed to edit NAT rule: %s", err)
							}
						}
					}
					...
					if len(strings.TrimSpace(rule[0])) > 63 {
						...
						if err != nil {
							if strings.Contains(err.Error(), "Client.Timeout") {
								...
								log.Printf("failed to edit Policy-Based Forwarding rule: %s", err)
							}
						}
					}
					...
					if len(strings.TrimSpace(rule[0])) > 63 {
						...
						if err != nil {
							if strings.Contains(err.Error(), "Client.Timeout") {
								...
								log.Printf("failed to edit Decryption rule: %s", err)
							}
						}
					}
			...
			if timeoutCount > 0 {
				log.Printf("There were %d API timeout errors during import. Please verify the following rules have been edited, modified:\n\n", timeoutCount)
		...
		case *pango.Panorama:
			...
			if t == "security" {
				...
				for _, rule := range rules {
					...
					if len(strings.TrimSpace(rule[2])) > 63 {
						log.Printf("failed to edit %s: name is over the max 63 characters", strings.TrimSpace(rule[2]))
					} else {
						e := security.Entry{
...
func init() {
	...
	policyEditCmd.Flags().StringVarP(&v, "vsys", "v", "vsys1", "Vsys name when editing on a firewall")
	policyEditCmd.Flags().StringVarP(&t, "type", "t", "", "Type of policy to edit - <security|nat|pbf|decrypt>")
	...
	// policyEditCmd.MarkFlagRequired("pass")
	// policyEditCmd.MarkFlagRequired("location")
}
